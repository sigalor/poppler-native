...
// project of the city of Munich Copyright (C) 2018 Thibaut Brard <thibaut.brard@gmail.com> Copyright (C) 2018, 2019
// Adam Reichold <adam.reichold@t-online.de> Copyright (C) 2019 Oliver Sander <oliver.sander@tu-dresden.de>
//
...
HtmlString::HtmlString(GfxState *state, double fontSize, HtmlFontAccu *_fonts) : fonts(_fonts) {
  ...
  if ((font = state->getFont())) {
    ...
    if (isMatRotOrSkew(state->getTextMat())) {
      ...
      if (debug) std::cerr << DEBUG << "before transform: " << print_matrix(normalizedMatrix) << std::endl;
      normalizedMatrix[1] *= -1;
      normalizedMatrix[2] *= -1;
      if (debug) std::cerr << DEBUG << "after reflecting angle: " << print_matrix(normalizedMatrix) << std::endl;
      normalizeRotMat(normalizedMatrix);
      if (debug) std::cerr << DEBUG << "after norm: " << print_matrix(normalizedMatrix) << std::endl;
...
HtmlPage::HtmlPage(bool newMergeLines, double newWordBreakThreshold)
    : mergeLines(newMergeLines), wordBreakThreshold(newWordBreakThreshold) {
  curStr = nullptr;
  yxStrings = nullptr;
  xyStrings = nullptr;
...
void HtmlPage::beginString(GfxState *state, const GooString *s) { curStr = new HtmlString(state, fontSize, fonts); }
...
void HtmlPage::addChar(GfxState *state, double x, double y, double dx, double dy, double ox, double oy,
...
// It may also strip out duplicate strings (if they are on top of each other); sometimes they are to create a font
// effect
void HtmlPage::coalesce() {
  ...
  while (str1 && (str2 = str1->yxNext)) {
    ...
    addLineBreak =
        mergeLines && (fabs(str1->xMin - str2->xMin) < 0.4) && IS_CLOSER(str2->yMax, str1->yMax + space, str1->yMax);
...
void HtmlPage::setDocName(const char *fname) { DocName = new GooString(fname); }
...
void HtmlOutputDev::drawChar(GfxState *state, double x, double y, double dx, double dy, double originX, double originY,
...
void HtmlOutputDev::drawJpegImage(GfxState *state, Stream *str) {
  ...
  // initialize and copy stream
  str = str->getNextStream();
  str->reset();
  while ((c = str->getChar()) != EOF) fputc(c, f1);
  fclose(f1);

  pages->addImage(new GooString(std::string("data:image/jpeg;base64,") + gbase64Encode(ims.getBuffer())), state);
}

void HtmlOutputDev::drawPngImage(GfxState *state, Stream *str, int width, int height, GfxImageColorMap *colorMap,
                                 bool isMask) {
#ifdef ENABLE_LIBPNG
  InMemoryFile ims;
  ...
  if (!isMask) {
    ...
    if (colorMap) {
      ...
      if (colToByte(gray) == 0) invert_bits = 0x00;
    }
    ...
    for (int ri = 0; ri < height; ++ri) {
      for (int i = 0; i < size; i++) png_row[i] = str->getChar() ^ invert_bits;
...
void HtmlOutputDev::drawImageMask(GfxState *state, Object *ref, Stream *str, int width, int height, bool invert,
...
void HtmlOutputDev::drawImage(GfxState *state, Object *ref, Stream *str, int width, int height,
...
GooString *HtmlOutputDev::getLinkDest(AnnotLink *link) {
  if (!link->getAction()) return new GooString();
...
int HtmlOutputDev::getOutlinePageNum(OutlineItem *item) {
  ...
  if (!action || action->getKind() != actionGoTo) return pagenum;

  link = dynamic_cast<const LinkGoTo *>(action);

  if (!link || !link->isOk()) return pagenum;
