...
inline bool is_within(double a, double thresh, double b) { return fabs(a - b) < thresh; }

inline bool rot_matrices_equal(const double *const mat0, const double *const mat1) {
  return is_within(mat0[0], .1, mat1[0]) && is_within(mat0[1], .1, mat1[1]) && is_within(mat0[2], .1, mat1[2]) &&
...
inline bool isMatRotOrSkew(const double *const mat) { return mat[0] < 0 || !is_within(mat[1], .1, 0); }
